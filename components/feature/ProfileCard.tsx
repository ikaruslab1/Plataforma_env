...
export function ProfileCard({ 
  ...
  return (
    <div className="max-w-md mx-auto w-full animate-in fade-in zoom-in-95 duration-700">
        ...
        {/* 3D Flip Container */}
        <div className="perspective-[1000px] w-full" onClick={() => !showWelcome && setIsFlipped(!isFlipped)}>
            <motion.div
                initial={false}
            ...
            >
                ...
                <div style={{ backfaceVisibility: "hidden" }}>
                    <Card className="overflow-hidden border-2 shadow-2xl print:shadow-none print:border-2 print:border-black h-full bg-white">
                        <div className="bg-brand-main text-primary-foreground p-8 text-center print:bg-black print:text-white">
            ...
            Mujeres y Niñas en la Ciencia</p>
                                <h2 className="text-3xl font-bold text-brand-darkest leading-tight">{displayName}</h2>
                                <div className="inline-block bg-brand-secondary/10 px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider text-brand-secondary border border-brand-secondary/20">
                                    {participacion}
                                </div>
                            </div>

                            <div className="flex justify-center p-4 bg-white rounded-xl border border-dashed border-gray-300 w-fit mx-auto shadow-sm group-hover:border-brand-main/50 transition-colors">
                                <QRCodeSVG 
                                    value={qrData} 
                                    size={160}
                                    level="Q"
            ...
            Unidad de Investigación Multidisciplinaria Aplicada
                ...
                >
                    <div className="p-8 flex flex-col items-center justify-center h-full space-y-6">
                         <img src="/ium-blanco.svg" alt="Logo UIM" className="w-48 h-auto opacity-95 drop-shadow-2xl" />
                    </div>
                </div>
            </motion.div>
        </div>
        ...
        <Dialog isOpen={showWelcome} onClose={closeWelcome} title="¡Registro Exitoso!">
            <div className="space-y-4 text-center">
                ...
                <div className="bg-brand-secondary/10 p-4 rounded-lg border border-brand-secondary/20">
                    <p className="font-semibold text-xl mb-1 text-brand-darkest">⚠️ IMPORTANTE</p>
                    <p className="text-xs text-brand-main/80">Guarda tu ID para iniciar sesión en el futuro. Es tu única llave de acceso.</p>
                </div>
                ...
                <div className="bg-secondary p-4 rounded-lg border border-border my-4">
                    ...
                    <p className="text-3xl font-bold tracking-wider text-brand-main mt-2 font-mono">{shortId}</p>
                </div>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Button className="w-full mt-2 bg-brand-main hover:bg-brand-main/90 text-white" onClick={closeWelcome}>
                        Entendido, continuar
                    </Button>
                </div>
            </div>
